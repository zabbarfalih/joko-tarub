$(document).ready(function () {
    ...
    $("#kalkulator-form-ipk").submit(function (event) {
        ...
        // Lakukan fetch data spreadsheet dan manipulasi data di dalam blok .then()
        fetch(FULL_URL)
            .then((res) => res.json())
            .then((data) => {
                ...
                // Temukan data wilayah yang sesuai dengan provinsi dari spreadsheet
                const dataProvinsi = dataWilayah.find(
                    (wilayah) => wilayah.wilayah === provinsi
                );

                // Ubah data di spreadsheet jika dataProvinsi ditemukan
                if (dataProvinsi) {
                    // Perbarui nilai x dari dataProvinsi dengan nilai IPK
                    dataProvinsi.x = IPK;

                    // Persiapkan data untuk dikirim kembali ke spreadsheet
                    const newData = dataWilayah.map((wilayah) => {
                        // Jika wilayah sama dengan provinsi yang diperbarui, gunakan nilai x yang baru
                        if (wilayah.wilayah === dataProvinsi.wilayah) {
                            return [wilayah.wilayah, IPK, wilayah.y];
                        } else {
                            return [wilayah.wilayah, wilayah.x, wilayah.y];
                        }
                    });

                    // Kirim data terbaru kembali ke spreadsheet menggunakan API Google Sheets
                    fetch(FULL_URL, {
